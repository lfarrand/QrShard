...
- Do not commit `.codex/config.toml` (machine-local Rider MCP), `.serena/`, `.cursor/hooks/state/`, `plan/*`, or `docs/reviews/*`.
...
- Windows sidecars `QrShard.Display` and `QrShard.Recorder` live in `QrShard.Windows.slnx` (`net10.0-windows`) with `EnableWindowsTargeting` on those csprojs only (PR 147) so Linux Automatic Dependency Submission `submit-nuget` does not hit NETSDK1100. `Vortice.Direct3D11` is pinned at 3.8.3 on `QrShard.Recorder` only. Testable internals are in `src/QrShard.Windows.Logic` (`net10.0`) on `QrShard.slnx` so Linux CI can test them.
